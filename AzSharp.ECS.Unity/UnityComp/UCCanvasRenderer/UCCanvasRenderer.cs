using AzSharp.Json.Serialization.Attributes;
using AzSharp.Json.Serialization.TypeSerializers;
using UnityEngine;
using UnityEngine.UI;
using AzSharp.ECS.Unity.Json.TypeSerializers.UnityComp;
using AzSharp.ECS.Shared.Components;

namespace AzSharp.ECS.Unity.UnityComp;

[JsonSerializable(typeof(UCCanvasRendererSerializer))]
[RegisterComponent(typeof(ComponentArray<UCCanvasRenderer>), typeof(ComponentEventRaiser<UCCanvasRenderer>), 48)]
public sealed class UCCanvasRenderer
{
    public CanvasRenderer? CanvasRenderer = null;
}
